defmodule Elementary.Printful.Parser do
  @moduledoc """
  ...
  def parse_product(product) do
    product
    |> Map.put(:category, parse_product_category(product))
  end
  ...
  def parse_variant(variant) do
    ...
    |> Map.put(:size, size)
    |> Map.put(:color, color)
    |> Map.put(:image, parse_variant_image(variant))
    |> Map.put(:thumbnail, parse_variant_thumbnail(variant))
  end
